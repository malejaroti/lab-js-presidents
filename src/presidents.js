...
function  countYearsInOffice(presidentsArr) {
    return presidentsArr.reduce(function(sum,p) {
      if(p.leftOffice !== null){
        return sum +  p.leftOffice - p.tookOffice;
      }
      else{
        return sum
      }
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map(p => {
    p["getAgeAtInauguration"]  = p.tookOffice - p.birthYear;
    return p;
  })
}
// console.table(getAgeAtInauguration(presidents))
...
// console.table(getPresidentsBornAfter(presidents, 1950))
...
function countRepublicanPresidents(presidentsArr) {
  const arr = presidentsArr.filter(p => p.party === "Republican")
  return arr.length
}
